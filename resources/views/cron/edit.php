<div class="page-card" style="max-width: 600px; padding: 32px;">
    <h2 style="margin-top: 0; font-size: 20px; display: flex; align-items: center; gap: 10px; font-weight: 600;">
        <i class="fa-solid fa-pen text-primary"></i> Редактировать задание
    </h2>
    <hr style="border: 0; border-top: 1px solid var(--border-color); margin: 20px 0 24px 0;">

    <form id="editCronForm" class="panel-form">
        ...
        <div class="form-group-row">
            ...
            <input type="email" name="email" placeholder="admin@example.com" value="<?= htmlspecialchars($job['email'] == '""' ? '' : $job['email']) ?>" <?= ($job['email'] == '""' || empty($job['email'])) ? 'disabled' : '' ?>>
            
            <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
                <input type="checkbox" name="no_email" id="noEmailCheck" style="width: auto;" <?= ($job['email'] == '""' || empty($job['email'])) ? 'checked' : '' ?>>
        ...
        <div class="form-group-row">
            ...
            <input type="text" name="command" placeholder="/usr/bin/php /var/www/.../script.php" value="<?= htmlspecialchars($job['command']) ?>" required>
        </div>
        ...
        <div class="form-group-row">
            ...
            <input type="text" name="description" placeholder="Моя задача..." value="<?= htmlspecialchars($job['description'] ?? '') ?>">
        </div>

        <div style="display: flex; align-items: center; gap: 10px; margin: 10px 0;">
            <input type="checkbox" name="is_active" id="isActiveCheck" style="width: auto;" <?= $job['is_active'] ? 'checked' : '' ?>>
        ...
        <div style="margin-top: 20px;">
            ...
            <div style="display: flex; gap: 24px; margin-bottom: 20px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="schedule_mode" id="modeExpert" value="expert" checked style="width: auto; cursor: pointer;">
                    <label for="modeExpert" style="font-size: 14px; cursor: pointer;">экспертный режим</label>
                </div>
            </div>

            <!-- Экспертный режим (только он при редактировании для простоты) -->
            <div id="expertScheduleWrapper" style="display: block; background: rgba(0,0,0,0.02); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <div class="form-group-row">
                    <label>CRON выражение</label>
                    <input type="text" name="schedule_expert" placeholder="0 * * * *" value="<?= htmlspecialchars($job['schedule']) ?>">
